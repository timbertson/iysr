	...
	where F: FnOnce(WorkerSelf<E>) -> Result<(),E>, F: Send + 'static, E: Send+Sync+'static
{
	...
	let builder = match name {
		None => builder,
		Some(ref name) => builder.name(name.clone()),
	};
	...
	self_thread_sender.send(self_thread).unwrap();

	match parent {
		Some(ref parent) => {
			match parent.lock() {
				Ok(ref mut parent) => {
					parent.add_child(shared.clone()).unwrap();
				},
				Err(_) => {
...
impl<E:Send + Sync + 'static> Worker<E> {
	...
	pub fn detach(&mut self) -> Result<(), TickError> {
		// TODO: detached threads will ignore errors, they should probably panic!
		let mut shared = try!(self.shared.lock());
		shared.detach()
	}

	pub fn terminate(&mut self) -> Result<(),WorkerError<E>> {
		fn err<Succ, Src,E>(_err:Src) -> Result<Succ, WorkerError<E>> {
			Err(WorkerError::Aborted("failed to send terminate()".to_string()))
		}

		let sent = self.shared.lock()
			.or_else(err)
			.and_then(|s|
				s.signal.send(()).or_else(err)
			);
